...
def main():
    ...
    run_command(f'brew bundle dump -f --describe --file={brewfile_path}/Brewfile', capture_output=True)

    result = subprocess.run(f'git status -s {brewfile_path}/Brewfile', shell=True, text=True, capture_output=True)
